...
from nacl.signing import SigningKey as NaClSigningSkey
...
def derive_public_key(signing_key: SigningKey) -> AccountNumber:
    return AccountNumber(
        bytes_to_hex(NaClSigningSkey(hex_to_bytes(signing_key)).verify_key))


def generate_key_pair() -> KeyPair:
    signing_key = NaClSigningSkey.generate()
    return KeyPair(
